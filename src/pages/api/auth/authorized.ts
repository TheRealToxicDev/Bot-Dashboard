...
import { encode, TokenData } from '../../../util/glob'
...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const data = {
    ...
    redirect_uri: 'http://localhost:3000/api/auth/authorized',
    code,
  ...
  if (!recievedData.access_token) {
    res.redirect(`/error?error=${encodeURIComponent('No access token was provided. Please try again later.')}`)
  }
